...
export class AnalyticsService {
  ...
  async getMetrics(): Promise<AnalyticsResponseDto> {
    ...
    const riskRows = rows.filter((row) => this.date(row.created_at) >= last24h);
    const waits = rows
      .filter((row) => row.source === 'patient-triage')
      .map((row) => {
        const end = row.completed_at ? this.date(row.completed_at) : now;
        return (end.getTime() - this.date(row.created_at).getTime()) / 60000;
      })
      .filter((minutes) => minutes >= 0);
    const reviewed = rows.filter((row) => row.ai_risk && row.final_risk);
    ...
    const hourlyVolume = Array.from(
      ...
      (_, hour) =>
        rows.filter(
          (row) =>
            this.date(row.created_at) >= today &&
            this.date(row.created_at).getHours() === hour,
        ).length,
    );
    const recentAverage = Math.max(0, Math.round(riskRows.length / 24));
    ...
    return {
      ...
      averageWaitMinutes: this.average(waits),
      averageWaitDelta: null,
      ...
      criticalCases: riskRows.filter(
        (row) => row.risk === 'ESI-1' || row.risk === 'ESI-2',
      ).length,
      criticalCasesDelta: null,
      riskDistribution: RISK_LEVELS.map((level) => ({
        level,
        count: riskRows.filter((row) => row.risk === level).length,
      })),
      hourlyVolume,
